...
const YearlyBreakup = () => {
    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            <DataCard data={revenue.data} trending={revenue.trending} icon={{ icon: UserPlus2 }} />
            <DataCard data={orders.data} trending={orders.trending} icon={{ icon: UserPlus2 }} />
            <DataCard data={users.data} trending={users.trending} icon={{ icon: UsersRound }} />
            <DataCard data={users.data} trending={users.trending} icon={{ icon: UsersRound }} />
        </div>
    );
};

function DataCard({ data, trending, icon }: { data: any; trending: any; icon: { icon: React.ElementType } }) {
    return (
